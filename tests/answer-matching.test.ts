...
describe("answer matching", () => {
  it("normalizes punctuation, casing, and diacritics", () => {
    expect(normalizeGuess("Maria Skłodowska-Curie")).toBe("maria sklodowska curie");
  ...
  it("matches aliases and canonical answers", () => {
    const marieCurie = demoSnapshot.entities.find((entity) => entity.id === "people-marie-curie");
    expect(marieCurie).toBeTruthy();

    expect(matchesEntityGuess(marieCurie!, "marie curie")).toBe(true);
    expect(matchesEntityGuess(marieCurie!, "Maria Sklodowska Curie")).toBe(true);

...
export function GameResultDialog({
  ...
  const [drawerState, setDrawerState] = useState<"medium" | "expanded">(
    "medium",
  );
  ...
  return (
    <div
      className="fixed inset-0 z-50 flex items-end justify-center bg-[rgba(25,20,14,0.46)] backdrop-blur-sm dark:bg-[rgba(3,7,14,0.62)] sm:p-4"
      onMouseDown={(event) => {
    ...
    >
      <div
        ...
        aria-modal="true"
        className={`relative w-full max-w-3xl overflow-y-auto rounded-t-4xl border border-b-0 border-[rgba(17,94,89,0.14)] bg-[linear-gradient(180deg,rgba(255,251,245,0.98),rgba(255,247,238,0.95))] shadow-[0_-20px_70px_rgba(29,22,14,0.26)] transition-[height,max-height] duration-300 ease-out dark:border-white/10 dark:bg-[linear-gradient(180deg,rgba(14,22,34,0.98),rgba(19,29,43,0.95))] dark:shadow-[0_-20px_70px_rgba(0,0,0,0.46)] sm:rounded-4xl sm:border-b ${
          isExpanded
            ? "h-[calc(100dvh-0.5rem)] max-h-[calc(100dvh-0.5rem)] sm:h-[min(920px,calc(100dvh-2rem))] sm:max-h-[calc(100dvh-2rem)]"
            : "max-h-[min(76dvh,680px)] sm:max-h-[min(76dvh,720px)]"
        }`}
        data-drawer-state={drawerState}
        role="dialog"
      >
        <div className="sticky top-0 z-20 flex h-12 items-center justify-center bg-[rgba(255,251,245,0.98)] dark:bg-[rgba(14,22,34,0.98)]">
          <button
            aria-expanded={isExpanded}
            aria-label={
              isExpanded ? "Collapse result drawer" : "Expand result drawer"
            }
            className="flex h-full w-28 items-center justify-center rounded-full focus-visible:outline-2 focus-visible:outline-offset-[-4px] focus-visible:outline-[#0f766e]"
            onClick={() =>
            ...
            type="button"
          >
            <span className="h-1.5 w-12 rounded-full bg-[#b8b0a4] dark:bg-[#65758a]" />
            <span className="sr-only">
              {isExpanded ? "Collapse" : "Expand"} result drawer
            </span>
          </button>
          <button
            aria-label="Close result"
            className="absolute right-4 top-1.5 inline-flex size-10 items-center justify-center rounded-full border border-black/8 bg-white/72 text-[#6b6259] transition hover:bg-white hover:text-[#1f1b17] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#0f766e] dark:border-white/10 dark:bg-white/6 dark:text-[#9aa9bb] dark:hover:bg-white/10 dark:hover:text-[#f5f7fb]"
            onClick={onClose}
            type="button"
          >
        ...
        <div className="p-6 pt-3 sm:p-7 sm:pt-3">

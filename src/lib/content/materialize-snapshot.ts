...
export function materializeSnapshot(
  ...
  const sourceFingerprint = hashString(stableStringify(sortedEntities.map((entity) => ({
    id: entity.id,
  ...
  const previousEntities = new Map(previousSnapshot?.entities.map((entity) => [entity.id, entity]) ?? []);
  const nextEntities = new Map(sortedEntities.map((entity) => [entity.id, entity]));

  const addedEntityIds = sortedEntities.filter((entity) => !previousEntities.has(entity.id)).map((entity) => entity.id);
  const updatedEntityIds = sortedEntities
    .filter((entity) => previousEntities.has(entity.id))
    .filter((entity) => previousEntities.get(entity.id)?.sourceFingerprint !== entity.sourceFingerprint)
